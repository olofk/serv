...
Converted to TL-Verilog (FEV-proven equivalent for W=1 and W=4, lint+sim clean):
`clr_lsb`, the `{c,q}` adder, the `c_r` carry register, `o_dbus_adr`, `o_ext_rs1`.
Remaining in `\SV`: module ports, `data` register declaration, the `clk` bridge,
and the W-parameterized `generate` datapath (see step 4). `fev.sh` + `regress.sh`
both pass.
